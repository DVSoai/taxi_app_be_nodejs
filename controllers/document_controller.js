...
module.exports.controller = (app, io, socket_list) => {
      ...
      checkAccessToken(
        ...
        (uObj) => {
          helper.CheckParameterValid(
            res,
            reqObj,
            ["doc_id", "zone_doc_id", "user_car_id", "expriry_date"],
            () => {
              helper.CheckParameterValid(res, files, ["image"], () => {
                ...
                var driver_doc_id = "";
                if (
                  reqObj.driver_doc_id != undefined ||
                  reqObj.driver_doc_id != null
                ) {
                  ...
                  (isDone, result) => {
                    if (!isDone) {
                      ...
                      if (reqObj["user_car_id"][0] == "") {
                        ...
                        db.query(
                          "INSERT INTO `zone_wise_doc_link`(`zone_doc_id`,`driver_doc_id`,`user_car_id`, `linked_date`) VALUES (?,?,?, NOW()) ",
                          [
                            reqObj.zone_doc_id[0],
                            result,
                            reqObj.user_car_id[0],
                          ],
                          (err, result) => {
                            if (err) {
                              helper.ThrowHtmlError(err, res);
    ...
    checkAccessToken(
      ...
      (uObj) => {
        helper.CheckParameterValid(res, reqObj, ["user_car_id"], () => {
          db.query(
            'SELECT IFNULL(`select_service_id`, "" ) AS `select_service_id` FROM `user_detail` WHERE `user_id` = ?',
            [uObj.user_id],
            (err, result) => {
              if (err) {
                helper.ThrowHtmlError(err, res);
              ...
              if (result.length > 0) {
                ...
                if (result[0].select_service_id != "") {
                  db.query(
                    "SELECT `zwd`.`zone_doc_id`, `zwd`.`service_id`, `zwd`.`personal_doc`, `zwd`.`car_doc`, `sd`.`service_name`, `sd`.`color` FROM `zone_document` AS `zwd`" +
                      "INNER JOIN`user_detail` AS`ud` ON`ud`.`zone_id` = `zwd`.`zone_id` AND`ud`.`user_id` = ? " +
                      "INNER JOIn`service_detail` AS`sd` ON`sd`.`service_id` = `zwd`.`service_id` AND`sd`.`status` = 1 " +
                      "AND`zwd`.`status` = 1 AND`zwd`.`service_id` IN(" +
                      result[0].select_service_id +
                      ") ",
                    [uObj.user_id],
                    (err, result) => {
                      if (err) {
                      ...
                      if (result.length > 0) {
                        ...
                        result.forEach((serviceDetail, index) => {
                          ...
                          var sql_doc =
                            "SELECT `doc_id`, `name`, `type`, `status`, `create_date`, `modify_date` FROM `document` WHERE `status` = 1 AND `doc_id` IN (" +
                            doc_id +
                            "); " +
                          ...
                          db.query(
                            ...
                            (err, docResult) => {
                              ...
                              docResult[1].forEach((carDetail, index) => {
                                ...
                                var sqlCarDoc =
                                  'SELECT `zwdl`.`zone_link_id`, `zwdl`.`driver_doc_id`, `zwdl`.`doc_status`, `zwdl`.`linked_date`, `dd`.`doc_id`, IFNULL( `dd`.`doc_image`, "" ) AS `doc_image`, `dd`.`expiry_date`, `dd`.`created_date`, `dd`.`status`, `d`.`type`, `d`.`name` AS `doc_name`  FROM `zone_wise_doc_link` AS `zwdl` ' +
                                ...
                                db.query(
                                  ...
                                  [
                                    uObj.user_id,
                                    carDetail.user_car_id,
                                    serviceDetail.zone_doc_id,
                                  ],
                                  (err, carDoc) => {
                                    if (err) {
                                      helper.ThrowHtmlError(err);
                                    ...
                                    for (
                                      let i = 0;
                                      i < docResult[0].length;
                                      i++
                                    ) {
                                      var isSet = false;
                                      for (let j = 0; j < carDoc.length; j++) {
                                        if (
                                          docResult[0][i].doc_id ==
                                          carDoc[j].doc_id
                                        ) {
  ...
  app.post("/api/personal_document_list", (req, res) => {
    ...
    checkAccessToken(req.headers, res, (uObj) => {
      db.query(
        "SELECT `zl`.`zone_name`, `zl`.`zone_id`, `zld`.`zone_doc_id`, `zld`.`service_id`, `sd`.`service_name`, `zld`.`personal_doc`, `d`.`name`, `d`.`type`, `d`.`doc_id`, `dd`.`doc_image`, `dd`.`expiry_date`, `dd`.`status`, `dd`.`created_date`, `dd`.`driver_doc_id` FROM `user_detail` AS `ud` " +
          ...
          "LEFT JOIN `driver_document` AS `dd` ON `dd`.`doc_id` = `d`.`doc_id` AND `dd`.`user_id` = `ud`.`user_id` AND `dd`.`status` != 1;",
        [uObj.user_id],
        (err, result) => {
          if (err) {
            helper.ThrowHtmlError(err, res);
          ...
          if (result.length > 0) {
            ...
            res.json({
              status: "0",
              message:
                "Please select any one zone & any one provide service type",
            });
          }
        }
      );
  ...
  app.post("/api/car_document_list", (req, res) => {
    ...
    checkAccessToken(req.headers, res, (uObj) => {
      helper.CheckParameterValid(res, reqObj, ["user_car_id"], () => {
        db.query(
          "SELECT `zl`.`zone_name`, `zl`.`zone_id`, `zld`.`zone_doc_id`, `zld`.`service_id`, `sd`.`service_name`, `zld`.`personal_doc`, `d`.`name`, `d`.`type`, `d`.`doc_id`, `dd`.`doc_image`, `dd`.`expiry_date`, `dd`.`status`, `dd`.`created_date`, `dd`.`driver_doc_id`, `zwdl`.`zone_link_id`, `zwdl`.`doc_status` FROM `user_detail` AS `ud`" +
            "INNER JOIN `zone_list` AS `zl` ON `zl`.`zone_id` = `ud`.`zone_id` AND `zl`.`status` = 1 AND `ud`.`user_id` = ? " +
            "INNER JOIN `zone_document` AS `zld` ON `zld`.`zone_id` = `zl`.`zone_id` AND FIND_IN_SET ( `zld`.`service_id`, `ud`.`select_service_id` ) > 0 AND `zld`.`status` = 1 " +
            "INNER JOIN `service_detail` AS `sd` ON `sd`.`service_id` = `zld`.`service_id` AND `sd`.`status` = 1 " +
          ...
          [uObj.user_id, reqObj.user_car_id],
          (err, result) => {
            if (err) {
              helper.ThrowHtmlError(err, res);
              return;
    ...
    checkAccessToken(
      ...
      (uObj) => {
        helper.CheckParameterValid(
          ...
          () => {
            db.query(
              "INSERT INTO `document`(`name`, `type`) VALUES (?,?) ",
              [reqObj.document_name, reqObj.document_type],
              (err, result) => {
                if (err) {
                  helper.ThrowHtmlError(err, res);
                  return;
  ...
  app.post("/api/admin/document_list", (req, res) => {
    checkAccessToken(
      ...
      (uObj) => {
        db.query(
          "SELECT `doc_id`, `name`, `type`, `status` FROM `document` WHERE `status` != ?",
          [2],
          (err, result) => {
            if (err) {
              helper.ThrowHtmlError(err, res);
    ...
    checkAccessToken(
      ...
      (uObj) => {
        helper.CheckParameterValid(res, reqObj, ["doc_id"], () => {
          db.query(
            "UPDATE `document` SET `status`=2, `modify_date`=NOW() WHERE `doc_id` = ? AND `status` != 2",
            [reqObj.doc_id],
            (err, result) => {
              if (err) {
                helper.ThrowHtmlError(err, res);
    ...
    checkAccessToken(
      ...
      (uObj) => {
        helper.CheckParameterValid(
          ...
          () => {
            db.query(
              "UPDATE `document` SET `name`=?, `type` = ?,  `modify_date`=NOW() WHERE `doc_id` = ? AND `status` != 2",
              [reqObj.document_name, reqObj.document_type, reqObj.doc_id],
              (err, result) => {
                if (err) {
                  helper.ThrowHtmlError(err, res);
                }
...
function documentUpload(
  ...
  if (driver_doc_id == undefined || driver_doc_id == "") {
    //
    var extension = image.originalFilename.substring(
    ...
    fs.rename(image.path, newPath, (err) => {
      if (err) {
        ...
        db.query(
          "INSERT INTO `driver_document`( `doc_id`, `user_id`, `doc_image`, `expiry_date`) VALUES (?,?,?, ?)",
          [doc_id, user_id, imageFileName, expriry_date],
          (err, result) => {
            if (err) {
              helper.ThrowHtmlError(err);
              return callback(false, "document upload fail");
    ...
    db.query(
      "SELECT * FROM `driver_document` WHERE `driver_doc_id` = ? AND `user_id` = ? AND `doc_id` = ? ",
      [driver_doc_id, user_id, doc_id],
      (err, result) => {
        if (err) {
          helper.ThrowHtmlError(err);
