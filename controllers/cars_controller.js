...
module.exports.controller = (app, io, socket_list) => {
      ...
      checkAccessToken(
        ...
        (uObj) => {
          helper.CheckParameterValid(
            res,
            reqObj,
            ["brand", "model", "series", "seat", "other_status", "car_number"],
            () => {
  ...
  app.post("/api/car_list", (req, res) => {
    checkAccessToken(
      ...
      (uObj) => {
        db.query(
          'SELECT  `uc`.`user_car_id`, `cs`.`series_name`, `cm`.`model_name`, `cb`.`brand_name`, `uc`.`car_number`, (CASE WHEN `uc`.`car_image` != ""  THEN CONCAT( "' +
            helper.ImagePath() +
            '" , `uc`.`car_image`  ) ELSE "" END) AS `car_image`, `uc`.`status`, `sd`.`service_name`, `sd`.`service_id`, `ud`.`select_service_id`, IFNULL(`zwcs`.`status`, 0) AS `service_status`,  (CASE WHEN `uc`.`user_car_id` = `ud`.`car_id` THEN 1 ELSE 0 END) AS `is_set_running`  FROM `user_cars` AS `uc` ' +
            ...
            " WHERE  `uc`.`user_id` = ? AND `uc`.`status` != ? GROUP BY `uc`.`user_car_id`, `sd`.`service_id` ORDER BY `uc`.`user_car_id`  ",
          [uObj.user_id, 2],
          (err, result) => {
            if (err) {
              helper.ThrowHtmlError(err, res);
            ...
            if (result.length > 0) {
              ...
              result.forEach((carDetail, index) => {
                ...
                if (index == 0) {
                  ...
                  carDetail.user_car_id != car_list[car_index].user_car_id
                ) {
                  car_list[car_index].service_missing_name = car_list[
                    car_index
                  ].service_missing_name.replace(/,\s*$/, "");
                  car_list.push(carDetail);
                  car_index++;
                  car_list[car_index].active_status = 1;
                  car_list[car_index].service_missing_name = "";
                ...
                if (carDetail.select_service_id != "") {
                  carDetail.select_service_id
                    .split(",")
                    .forEach((series_id) => {
                      if (
                        carDetail.service_status == 0 &&
    ...
    checkAccessToken(
      ...
      (uObj) => {
        helper.CheckParameterValid(res, reqObj, ["user_car_id"], () => {
          db.query(
            "UPDATE  `user_cars` SET `status` = ? WHERE `user_car_id` = ? AND `user_id` = ? ",
            [2, reqObj.user_car_id, uObj.user_id],
            (err, result) => {
              if (err) {
                helper.ThrowHtmlError(err, res);
                return;
        ...
        helper.CheckParameterValid(res, reqObj, ["user_car_id"], () => {
          db.query(
            "UPDATE `user_cars` AS `ucd` " +
              ...
              "WHERE `ucd`.`user_car_id` = ? AND `ucd`.`user_id` = ? AND `ucd`.`status` = ?",
            [reqObj.user_car_id, uObj.user_id, 1],
            (err, result) => {
              if (err) {
                helper.ThrowHtmlError(err, res);
                return;
              ...
              if (result.affectedRows > 0) {
                db.query(
                  'SELECT  `uc`.`user_car_id`, `cs`.`series_name`, `cm`.`model_name`, `cb`.`brand_name`, `uc`.`car_number`,  (CASE WHEN `uc`.`car_image` != ""  THEN CONCAT( "' +
                    helper.ImagePath() +
                    '" , `uc`.`car_image`  ) ELSE "" END) AS `car_image`, `uc`.`status`,(CASE WHEN `uc`.`user_car_id` = `ud`.`car_id` THEN 1 ELSE 0 END) AS `is_set_running`  FROM `user_cars` AS `uc` ' +
                    "INNER JOIN `car_series` AS `cs` ON  `uc`.`series_id` = `cs`.`series_id` " +
                    "INNER JOIN `car_model` AS `cm` ON  `cm`.`model_id` = `cm`.`model_id` " +
                    "INNER JOIN `car_brand` AS `cb` ON `cb`.`brand_id` = `cm`.`brand_id`  " +
                    "INNER JOIN `user_detail` AS `ud` ON `ud`.`user_id` = `uc`.`user_id` " +
                    "WHERE `uc`.`user_car_id` = ? AND `uc`.`status` = ? ",
                  [reqObj.user_car_id, 1],
                  (err, result) => {
                    if (err) {
                      helper.ThrowHtmlError(err, res);
                      return;
                    ...
                    if (result.length > 0) {
                      ...
                      res.json({
                        status: "1",
                        payload: result,
                        message: "car set running succfully",
                      });
                    } else {
                      res.json({ status: "0", message: msg_fail });
                    }
  ...
  app.post("/api/brand_list", (req, res) => {
    checkAccessToken(req.headers, res, (uObj) => {
      db.query(
        "SELECT `brand_id`, `brand_name` FROM `car_brand` WHERE `status` != ?",
        [2],
        (err, result) => {
          if (err) {
            helper.ThrowHtmlError(err, res);
  ...
  app.post("/api/model_list", (req, res) => {
    ...
    checkAccessToken(req.headers, res, (uObj) => {
      helper.CheckParameterValid(res, reqObj, ["brand_id"], () => {
        db.query(
          "SELECT `cm`.`model_id`, `cm`.`model_name`, `cm`.`seat` FROM `car_model` AS `cm` " +
  ...
  app.post("/api/series_list", (req, res) => {
    ...
    checkAccessToken(req.headers, res, (uObj) => {
      helper.CheckParameterValid(res, reqObj, ["model_id"], () => {
        db.query(
          "SELECT  `cs`.`series_id`, `cs`.`series_name` FROM `car_series` AS `cs` " +
  ...
  app.post("/api/subscription_plan_list", (req, res) => {
    ...
    checkAccessToken(req.headers, res, (uObj) => {
      db.query(
        " SELECT `sp`.`plan_id`, `sp`.`plan_name`,`sp`.`detail`, `sp`.`days`, `sp`.`amount`,`sp`.`max_discount`,`sp`.`max_ride`, `sp`.`zone_id`, `sp`.`service_id`, `sp`.`min_amount`, `sp`.`discount_per`, `sp`.`image`, `sb`.`user_typ`, `sp`.`start_date`, `sp`.`end_date`, `sp`.`created_date`, `zl`.`zone_name`, GROUP_CONCAT(`sp`.`service_name`) AS `service_name`     FROM `subscription_plan` AS `sp` " +
          ...
          "WHERE `sp`.`status` = 1 AND `sp`.`start_date` >= NOW() AND `sp`.`end_date` <= NOW() GROUP BY `sp`.`plan_id` ",
        [],
        (err, result) => {
          if (err) {
            helper.ThrowHtmlError(err, res);
    ...
    checkAccessToken(
      ...
      (uObj) => {
        helper.CheckParameterValid(
          res,
          reqObj,
          ["brand", "model", "series", "seat", "other_status"],
          () => {
            var responseObj = { status: "0", message: "Car add fail" };
  ...
  app.post("/api/admin/brand_list", (req, res) => {
    checkAccessToken(
      ...
      (uObj) => {
        db.query(
          "SELECT `brand_id`, `brand_name`, `status`, `created_date`, `modify_date` FROM `car_brand` WHERE `status` != ?",
          [2],
          (err, result) => {
            if (err) {
  ...
  app.post("/api/admin/model_list", (req, res) => {
    checkAccessToken(
      ...
      (uObj) => {
        db.query(
          "SELECT `cm`.`model_id`, `cb`.`brand_name`, `cm`.`brand_id`, `cm`.`model_name`, `cm`.`seat`, `cm`.`status`, `cm`.`created_date`, `cm`.`modify_date` FROM `car_model` AS `cm` " +
            "INNER JOIN `car_brand` AS `cb` ON `cb`.`brand_id` = `cm`.`brand_id` " +
            " WHERE `cm`.`status` != ?",
          [2],
          (err, result) => {
  ...
  app.post("/api/admin/series_list", (req, res) => {
    checkAccessToken(
      ...
      (uObj) => {
        db.query(
          "SELECT `cm`.`model_id`, `cm`.`model_name`, `cb`.`brand_name`, `cm`.`brand_id`, `cs`.`series_id`,  `cs`.`series_name`, `cs`.`status`, `cs`.`created_date`, `cs`.`modify_date` FROM `car_series` AS `cs` " +
        ...
        helper.CheckParameterValid(res, reqObj, ["brand_id"], () => {
          db.query(
            "UPDATE `car_brand` AS `cb` " +
              ...
              " WHERE `cb`.`brand_id` = ? AND `cb`.`status` != ? ",
            [reqObj.brand_id, 2],
            (err, result) => {
              if (err) {
                helper.ThrowHtmlError(err, res);
        ...
        helper.CheckParameterValid(res, reqObj, ["model_id"], () => {
          db.query(
            "UPDATE `car_model` AS `cm` " +
              ...
              " WHERE `cm`.`model_id` = ? AND `cm`.`status` != ? ",
            [reqObj.model_id, 2],
            (err, result) => {
              if (err) {
                helper.ThrowHtmlError(err, res);
        ...
        helper.CheckParameterValid(res, reqObj, ["series_id"], () => {
          db.query(
            "UPDATE `car_series` AS `cs` " +
              ...
              " WHERE `cs`.`series_id` = ? AND `cs`.`status` != ? ",
            [reqObj.series_id, 2],
            (err, result) => {
              if (err) {
                helper.ThrowHtmlError(err, res);
  ...
  app.post("/api/admin/series_delete", (req, res) => {
    ...
    checkAccessToken(
      ...
      (uObj) => {
        helper.CheckParameterValid(res, reqObj, ["series_id"], () => {
          db.query(
            "UPDATE `car_series` AS `cs` SET `cs`.`modify_date` = NOW(), `cs`.`status` = 2 " +
              " WHERE `cs`.`series_id` = ? ",
            [reqObj.series_id],
            (err, result) => {
              if (err) {
          ...
          helper.CheckParameterValid(
            ...
            () => {
              helper.CheckParameterValid(res, files, ["image"], () => {
                ...
                var imageFileName =
                  "subscription_plan/" + helper.fileNameGenerate(extension);
                ...
                fs.rename(files.image[0].path, newPath, (err) => {
                  if (err) {
                    ...
                    db.query(
                      "INSERT INTO `subscription_plan`( `plan_name`, `detail`, `days`, `amount`, `max_discount`, `max_ride`, `zone_id`, `service_id`, `min_amount`, `discount_per`, `image`, `user_type`, `start_date`, `end_date`) VALUES (?,?,?, ?,?,?, ?,?,?, ?,?,?, ?,? )",
                      [
                        reqObj.plan_name[0],
                        reqObj.plan_details[0],
                        reqObj.days[0],
                        reqObj.amount[0],
                        reqObj.max_discount[0],
                        reqObj.max_ride[0],
                        reqObj.zone_id[0],
                        reqObj.service_id[0],
                        reqObj.min_amount[0],
                        reqObj.discount_per[0],
                        imageFileName,
                        reqObj.user_type[0],
                        reqObj.start_date[0],
                        reqObj.end_date[0],
        ...
        (uObj) => {
          helper.CheckParameterValid(
            ...
            () => {
              ...
              db.query(
                "UPDATE `subscription_plan` SET `plan_name` = ?, `detail`= ?, `days`= ?, `amount`= ?, `max_discount`= ?, `max_ride`= ?, `zone_id`= ?, `service_id`= ?, `min_amount`= ?, `discount_per`= ?, " +
                  condition +
                  " `user_type` = ? , `start_date` = ?, `end_date` = ? WHERE `plan_id` = ? AND `start_date` > NOW() ",
                [
                  reqObj.plan_name[0],
                  reqObj.plan_details[0],
                  reqObj.days[0],
    ...
    checkAccessToken(
      ...
      (uObj) => {
        helper.CheckParameterValid(res, reqObj, ["plan_id"], () => {
          db.query(
            "UPDATE `subscription_plan` SET `status` = ?, `modify_date`= ? WHERE `plan_id` = ? ",
            [reqObj.plan_id],
            (err, result) => {
              if (err) {
                helper.ThrowHtmlError(err, res);
      ...
      (uObj) => {
        db.query(
          "SELECT `sp`.`plan_id`, `sp`.`plan_name`, `sp`.`detail`, `sp`.`days`, `sp`.`amount`, `sp`.`max_discount`, `sp`.`max_ride`, `sp`.`zone_id`, `sp`.`service_id`, `sp`.`min_amount`, `sp`.`discount_per`, `sp`.`image`, `sp`.`user_type`, `sp`.`start_date`, `sp`.`end_date`, `sp`.`status`, `sp`.`created_date`, `sp`.`modify_date`, GROUP_CONCAT(`sd`.`service_name` ) AS `service_name`, `zl`.`zone_name` FROM `subscription_plan` AS `sp` " +
            ...
            "SELECT `zone_id`, `zone_name` FROM `zone_list` WHERE `status` != 2;",
          [],
          (err, result) => {
            if (err) {
              helper.ThrowHtmlError(err, res);
...
function car_brand_add(car_brand, callback) {
  db.query(
    "SELECT `brand_id`, `brand_name`, `status`, `created_date`, `modify_date` FROM `car_brand` WHERE `brand_name` = ?",
    [car_brand.toUpperCase()],
    (err, result) => {
      if (err) {
        helper.ThrowHtmlError(err);
        return;
      ...
      if (result.length > 0) {
        //Exits
        db.query(
          'UPDATE `car_brand` SET `modify_date` = (CASE WHEN  `status` = "2" THEN  NOW() ELSE `modify_date` END), `status` = (CASE WHEN  `status` = "2" THEN  0 ELSE `status` END) WHERE `brand_id` = ? ',
          [result[0].brand_id],
          (err, result) => {
            if (err) {
              helper.ThrowHtmlError(err);
...
function car_model_add(brand_id, car_model, seat, callback) {
  db.query(
    "SELECT `model_id`, `brand_id`, `model_name`, `seat`, `status`, `created_date`, `modify_date` FROM `car_model` WHERE `brand_id` = ? AND `model_name` = ? AND `seat` = ?",
    [brand_id, car_model.toUpperCase(), seat],
    (err, result) => {
      if (err) {
        helper.ThrowHtmlError(err);
        return;
      ...
      if (result.length > 0) {
        ...
        //Add New
        db.query(
          "INSERT INTO `car_model` ( `brand_id`, `model_name`, `seat` ) VALUES (?,?,?)",
          [brand_id, car_model.toUpperCase(), seat],
          (err, result) => {
            if (err) {
              helper.ThrowHtmlError(err);
...
function car_series_add(brand_id, model_id, car_series, callback) {
  db.query(
    "SELECT `series_id`, `brand_id`, `model_id`, `series_name`, `status`, `created_date`, `modify_date` FROM `car_series` WHERE `brand_id` = ? AND `model_id` = ? AND `series_name` = ?",
    [brand_id, model_id, car_series.toUpperCase()],
    (err, result) => {
      if (err) {
        helper.ThrowHtmlError(err);
        return;
      ...
      if (result.length > 0) {
        ...
        //Add New
        db.query(
          "INSERT INTO `car_series`( `brand_id`, `model_id`, `series_name` ) VALUES (?,?,?)",
          [brand_id, model_id, car_series],
          (err, result) => {
            if (err) {
              helper.ThrowHtmlError(err);
              return;
...
function user_car_add(
  ...
  db.query(
    "SELECT `user_car_id` FROM `user_cars` WHERE `user_id` = ? AND `series_id` = ? AND `car_number` = ? AND `status` != 2 ",
    [user_id, series_id, car_number],
    (err, result) => {
      if (err) {
        helper.ThrowHtmlError(err);
      ...
      if (result.length == 0) {
        var extension = car_image_path.originalFilename.substring(
          car_image_path.originalFilename.lastIndexOf(".") + 1
        );
        ...
        db.query(
          "INSERT INTO `user_cars`( `user_id`, `series_id`, `car_number`, `car_image`) VALUES (?,?,?, ? )",
